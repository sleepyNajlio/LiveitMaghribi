...
import { Pi, Pickaxe } from "lucide-react";
import { Button } from "../ui/button";

export const WorkshopZ = ({ workshops, onCardClick }) => {
  ...
  return (
    <div className="flex flex-col justify-start gap-6 p-1">
      {workshops.map((workshop, idx) => (
        <div
          key={idx}
          className="relative flex flex-col md:flex-row items-center justify-center bg-foreground rounded-2xl text-card-foreground"
          style={{
        ...
        >
          <div className="w-full h-full md:w-1/2  flex flex-row md:flex-col justify-center items-center">
            <div
              className="w-1/2 aspect-[2/3] max-w-sm flex flex-col items-center justify-center relative z-10 overflow-hidden "
              style={{
            ...
            <h2 className="text-xl font-extrabold italic mb-2 font-playfair w-1/2 md:w-full text-left tracking-widest leading-loose text-balance drop-shadow-md md:text-center ">
              {workshop.title}
            </h2>
          </div>
          <div className="w-full h-full flex flex-col items-center justify-center  rounded-xl px-4 pb-4 text-pretty text-left gap-4 max-w-md">
            {/* <h2 className="text-2xl font-bold mb-2 font-playfair place-self-end max-w-[1/2]">
              {workshop.subline}
            </h2> */}
            <p className="tracking-wide text-base leading-relaxed drop-shadow-lg text-center">
              {workshop.description}
            </p>
            <div className="flex flex-col items-start justify-start">
              <Pickaxe className=" text-accent  block w-11 h-11 ml-[-5px] mr-2 mt-[-8px] place-self-center" />
            ...
            <div className="flex flex-col items-start justify-center gap-2">
              {workshop.process.map((step, id) => (
                <div
                  ...
                  key={id}
                >
                  <FaCheckCircle className="block text-accent  min-w-6 min-h-6 max-w-6 max-h-6 ml-[-5px] mr-2 mt-1  place-self-start" />
            ...
            <div className="flex flex-col flex-center items-center justify-center leading-relaxed">
              ...
              <div className="flex flex-col gap-2 items-start justify-start leading-relaxed">
                {workshop.notice.options.map((option, id) => (
                  <div
                    ...
                    className="flex flex-row items-center justify-center leading-relaxed"
                  >
                    <GiPaintedPottery className="text-accent min-w-11 min-h-11 max-w-11 max-h-11 ml-[-5px] mr-2 mt-1 " />
                    <p>{option}</p>
                  </div>
                ))}
              </div>
              <div className="flex flex-col items-center justify-center leading-relaxed gap-6 py-4">
                <p className="font-bold text-base text-center leading-relaxed">
                ...
                <div className="flex flex-row items-center justify-between gap-4">
                  ...
                  <Button variant="secondary" className=" px-6 py-4 rounded-full">
